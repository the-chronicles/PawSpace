...
import { collection, addDoc, query, where, getDocs, doc, getDoc, orderBy, limit, Timestamp, updateDoc } from 'firebase/firestore';
import { ref, uploadBytes, getDownloadURL } from 'firebase/storage';
import { getFirebaseFirestore, getFirebaseStorage } from '@/firebase/config';
import { ListingItem } from '@/types';
...
export async function getListings(): Promise<ListingItem[]> {
  ...
  const listings: ListingItem[] = snapshot.docs.map((doc) => ({
    id: doc.id,
    ...(doc.data() as Omit<ListingItem, 'id'>),
  }));
...
export async function getListingById(id: string): Promise<ListingItem> {
  ...
  return {
    id: snapshot.id,
    ...(snapshot.data() as Omit<ListingItem, 'id'>),
  };
}
